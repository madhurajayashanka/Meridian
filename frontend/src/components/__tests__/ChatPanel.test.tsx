...
describe('ChatPanel Component', () => {
  ...
  it('should disable input while sending message', async () => {
    ...
    const input = screen.getByPlaceholderText(/ask|message|query/i) as HTMLInputElement;
    const submitButton = screen.getByRole('button', { name: /send|submit/i });
  ...
  it('should display loading indicator during streaming', async () => {
    ...
    expect(screen.getByPlaceholderText(/ask|message|query/i)).toBeInTheDocument();
  });

  it('should persist messages across re-renders', async () => {
    const user = userEvent.setup();
    const { rerender } = render(<ChatPanel reportId={mockReportId} jobId={mockJobId} />);
  ...
  it('should support multiline input', async () => {
    ...
    const input = screen.getByPlaceholderText(/ask|message|query/i) as HTMLTextAreaElement;
