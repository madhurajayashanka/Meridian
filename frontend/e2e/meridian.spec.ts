...
test.describe('Meridian Platform E2E Tests', () => {
  ...
  test.describe('Project Management', () => {
    test('should create a new project', async ({ page, context }) => {
      ...
      await page.fill('textarea[name="description"]', 'A test research project');
    ...
    test('should view project details', async ({ page }) => {
      ...
      const projectCard = await page.locator('[role="link"]:has-text("Test Project")').first();
  ...
  test.describe('Research Job Workflow', () => {
    test('should submit a research query', async ({ page }) => {
      ...
      const newResearchBtn = await page.locator('button:has-text("New Research")').first();
      if (newResearchBtn) {
      ...
      if (queryField) {
        await queryField.fill('What are the latest trends in artificial intelligence?');

        // Select LLM provider
        const providerSelect = await page.locator('select[name="llmProvider"]').first();
        if (providerSelect) {
    ...
    test('should monitor job progress', async ({ page }) => {
      ...
      const statusText = await page.locator('text=/Running|Complete|Failed/').first();
      if (statusText) {
  ...
  test.describe('Report Viewing', () => {
    test('should view report with markdown content', async ({ page }) => {
      ...
      const toc = await page.locator('nav[aria-label="Table of contents"]').first();
      if (toc) {
    ...
    test('should navigate using table of contents', async ({ page }) => {
      ...
      // Click a TOC item
      const tocLink = await page.locator('nav[aria-label="Table of contents"] a').first();
      if (tocLink) {
        await tocLink.click();
    ...
    test('should interact with chat panel', async ({ page }) => {
      ...
      // Find chat input
      const chatInput = await page.locator('input[placeholder*="Ask"], textarea[placeholder*="Ask"]').first();
      if (chatInput) {
        await chatInput.fill('What is the main topic?');

        // Send message
        const sendBtn = await page.locator('button:has-text("Send"), button[type="submit"]').last();
        await sendBtn.click();

        // Message should appear
        await expect(page.locator('text=What is the main topic?')).toBeVisible();
      }
    });
  });
  ...
  test.describe('Document Upload', () => {
    ...
    test('should show upload progress', async ({ page }) => {
      ...
      if (fileInput) {
        ...
        const progress = await page.locator('[role="progressbar"], [class*="progress"]').first();
        if (progress) {
